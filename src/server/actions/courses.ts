...
function ownedCourse(ctx: AuthContext, id: string): Course {
...
function uniqueCourseSlug(ctx: AuthContext, base: string): string {
  ...
  try {
    ...
    const created = db
      ...
      .values({
        ...
        title,
        slug: uniqueCourseSlug(ctx, title),
    ...
    db.insert(courseModules).values({ courseId: created.id, title: 'Getting started', position: 0 }).run();
    audit('course.created', { actorUserId: ctx.user.id, target: created.id, meta: { title } });
...
export async function updateCourseAction(
  ...
  try {
    ...
    db.update(courses).set(updates).where(eq(courses.id, id)).run();

    // Keep marketplace listing in sync
    const listing = db.select().from(marketplaceListings).where(eq(marketplaceListings.courseId, id)).get();
    if (listing) {
...
export async function deleteCourseAction(id: string): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
    ...
    audit('course.deleted', { actorUserId: ctx.user.id, target: id });
    revalidatePath('/dashboard/courses');
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : 'Failed' };
...
export async function addModuleAction(
  ...
  try {
    const ctx = await requireUser();
    ownedCourse(ctx, courseId);
    const count = db.select({ id: courseModules.id }).from(courseModules).where(eq(courseModules.courseId, courseId)).all().length;
    const row = db
...
export async function renameModuleAction(
  ...
  try {
    const ctx = await requireUser();
    const mod = db.select().from(courseModules).where(eq(courseModules.id, moduleId)).get();
    if (!mod) throw new Error('Module not found');
    ownedCourse(ctx, mod.courseId);
    db.update(courseModules).set({ title: title.slice(0, 120) }).where(eq(courseModules.id, moduleId)).run();
...
export async function deleteModuleAction(
  ...
  try {
    ...
    if (!mod) throw new Error('Module not found');
    ownedCourse(ctx, mod.courseId);
    db.delete(courseModules).where(eq(courseModules.id, moduleId)).run();
    revalidatePath(`/dashboard/courses/${mod.courseId}`);
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : 'Failed' };
...
export async function addLessonAction(
  ...
  try {
    const ctx = await requireUser();
    ownedCourse(ctx, courseId);
    const count = db.select({ id: lessons.id }).from(lessons).where(eq(lessons.courseId, courseId)).all().length;
    const row = db
      .insert(lessons)
      .values({
        courseId,
...
export async function updateLessonAction(
  ...
  try {
    ...
    ownedCourse(ctx, lesson.courseId);
    ...
    db.update(lessons).set(updates).where(eq(lessons.id, lessonId)).run();
...
export async function markLessonCompleteAction(
  ...
  try {
    ...
    const total = db.select({ id: lessons.id }).from(lessons).where(eq(lessons.courseId, courseId)).all().length;
    const done = db
